...
class RectangleCollider_QT{
  private:
    ...
    vector<vector<int>> groupsForQuad(const vector<Rect> rects_qg, Rect bounds){ // fix this
  ...
  public:
    ...
    vector<pair<int, int>> intersections_qt(int depth, Rect bounds, const vector<Rect> rects_qt){
      if(depth == 0 || rects_qt.size() <= 3){
          vector<pair<int, int>> intersections;
          for(int i = 0; i < rects_qt.size(); ++i){
              for(int j = i + 1; j < rects_qt.size(); ++j){
                  if(collide_2d(rects_qt[i], rects_qt[j])){
                      intersections.push_back({i, j});
                  }
              }
          }
...
int main(){
  ...
  RectangleCollider_QT rectcolqt;
  ...
  for(int i = 0; i < case_num; ++i){
      ...
      start_time = high_resolution_clock::now();
      cout << rectcolqt.intersections_qt(0, bound, rectcolqt.ret_rects()).size();
      stop_time = high_resolution_clock::now();
      duration = duration_cast<microseconds>(stop_time - start_time);
      cout << ", " << duration.count() << endl;
